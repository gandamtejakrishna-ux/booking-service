...
//    val fetchQuery =
//      sql"""SELECT id, payload FROM outbox_events WHERE published = FALSE ORDER BY id LIMIT 20"""
//        .as[(Long, String)]
//
//    db.run(fetchQuery).map { rows =>
//      rows.foreach { case (id, payloadStr) =>
//        val json = play.api.libs.json.Json.parse(payloadStr)
//        publishEvent(id, json)
//      }
//    }
//  }
//
//  private def publishEvent(id: Long, payload: play.api.libs.json.JsValue): Unit = {
//    publisher.publish(payload).map { _ =>
//      val updateQuery = sqlu"UPDATE outbox_events SET published = TRUE WHERE id = $id"
//      db.run(updateQuery).map { _ =>
//        logger.info(s"Outbox event $id marked as published")
//      }
//    }.recover { case ex =>
//      logger.error(s"Outbox $id failed to publish. Will retry. Error=${ex.getMessage}")
//    }
//  }
//}
package com.hotel.booking.outbox
